...
// @desc    Register new tenant organization + owner user
// @access  Protected by SIGNUP_SECRET_KEY (set in .env)
//          Only your onboarding flow / admin should know this key.
router.post('/signup', async (req, res) => {
  try {
    // Gate: require the signup secret to prevent self-registration abuse
    const signupKey = req.headers['x-signup-key'] || req.body.signupKey;
    if (!process.env.SIGNUP_SECRET_KEY || signupKey !== process.env.SIGNUP_SECRET_KEY) {
      return res.status(403).json({ message: 'Unauthorized: invalid or missing signup key.' });
    }
